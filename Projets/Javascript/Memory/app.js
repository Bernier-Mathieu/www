...
function clicCanvas(event){
    ...
    if(clicReplay(x,y)){newParty();return;}
...
function selectCard(indexCard){
    if(app.score <= 0 || app.idCardsSelected.includes(indexCard)||app.idCardsFound.includes(indexCard)) 
        return;
...
function checkPair() {
 ...
 function render(){
    app.context.drawImage(app.images["memory"],0,0,
